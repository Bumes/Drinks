...
function Drink(category, name, ingredients=[], garnish="") {
    if (ingredients.length != 0){
        ...
        for (let i = 0; i < ingredients.length; i++) {
            ...
            formatted_ingredient = ingredient.toLowerCase().split("//")[0].replace("double ", "").replace("steamed ", "").replace(/[\d½|\d¼]+(ml|g)? /, '').replace(/ /g, '_').replace(/[()]/g, '').replace(/([^ ])_+([^ ])/g, (match, group1, group2) => group1 + group2);
